...
from sqlalchemy import create_engine, text, MetaData, Table, Column, Integer, String, DateTime, Boolean, Float, Select
from sqlalchemy.orm import sessionmaker, Session, declarative_base
from sqlalchemy.sql import select, insert, update, delete, and_, or_, not_
        ...
        if self._conditions:
            ...
            for cond in self._conditions:
                ...
                if op in ["IS NULL", "IS NOT NULL"]:
                    ...
                    value_repr = f"'{cond['value']}'" if isinstance(cond["value"], str) else str(cond["value"])
...
class DBKernel:
    ...
    def __init__(self, db_url: str = "sqlite:///app.db", cache_max_size: int = 1000, cache_ttl: int = 300):
        self.db_url = db_url
        self.engine = None
    ...
    def initialize(self) -> None:
        ...
        logger.info(f"Кэширование запросов включено (TTL={self._cache_ttl}s, max_size={len(self._query_cache)})")
    ...
    def build_query(self, table: str, conditions: List[Dict] = None, 
                   fields: List[str] = None) -> QueryBuilder:
    ...
    def execute_transaction(self, operations: List[Dict[str, Any]]) -> List[Any]:
        ...
        try:
            ...
            for op in operations:
                ...
                if op_type == "insert":
                    results.append({"id": result.inserted_primary_key[0] if result.inserted_primary_key else None})
                elif op_type == "update":
    ...
    def enable_cache(self, ttl: Optional[int] = None) -> None:
...
def create_db_kernel(db_url: str = "sqlite:///app.db", cache_max_size: int = 1000, cache_ttl: int = 300) -> DBKernel:
    """
