...
class TestUpdaterModule:
    ...
    def test_check_via_web_success(self, mock_urlopen):
        ...
        # Мокируем проверку доступности ассета
        with patch.object(self.updater, '_asset_available', return_value=True):
            with patch.object(self.updater, '_platform_asset', return_value="FinanceCalculator-linux"):
    ...
    def test_resolve_download_url_from_assets(self):
        ...
        release_info = {
            "assets": [
                {"name": "FinanceCalculator-windows.exe", "browser_download_url": "https://example.com/win.exe"},
                {"name": "FinanceCalculator-macos", "browser_download_url": "https://example.com/mac"},
                {"name": "FinanceCalculator-linux", "browser_download_url": "https://example.com/linux"}
            ]
        }
    ...
    def test_pick_release_skips_drafts(self):
        ...
        releases = [
            ...
            {"tag_name": "v2.0.0", "draft": False, "assets": [{"name": "test.zip", "browser_download_url": "https://example.com/test.zip"}]}
        ]
    ...
    def test_calculate_checksum(self):
        ...
        try:
            ...
            assert checksum == "c59d9b55f95a49cef1be6d56d0ec1e3ace1cc13d12df5bc871a7649ed155c48b"
        finally:
            temp_path.unlink()
    ...
    def test_download_with_progress(self, mock_urlopen):
        ...
        # Мокируем ответ с файлом
        mock_response = MagicMock()
        mock_response.getheader.return_value = "100"  # Content-Length
        mock_response.read.side_effect = [b"x" * 50, b"x" * 50, b""]  #chunks
        mock_response.__enter__ = lambda s: s
        mock_response.__exit__ = lambda s, *args: None
        mock_urlopen.return_value = mock_response
    ...
    def test_run_update_check_auto_download(self):
        ...
        with patch.object(self.updater, 'check_for_updates', return_value=(True, "2.0.0", "https://example.com/update.zip")):
            with patch.object(self.updater, 'download_update', return_value=True) as mock_download:
...
class TestCheckUpdatesFunction:
    ...
    def test_check_updates_marks_checked(self, mock_updater_class, mock_recent):
        ...
        with patch('modules.updater.updater.read_version', return_value="1.0.0"):
            with patch('modules.updater.updater._mark_checked') as mock_mark:
