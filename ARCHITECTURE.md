...
(таблица income) − расходы − плановый платёж по непогашенным долгам`
(`debts.monthly_payment`, только пока `remaining_amount > 0`, по
`debts.payment_half`). Несколько источников дохода на одну половину
суммируются; повторяющиеся доходы проецируются вперёд как и расходы.
Расчёт зарплаты с отпуском — по ТК РФ: оклад пропорционально отработанным
дням, отпускные начисляются отдельной суммой.
