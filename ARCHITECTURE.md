...
На каждую половину месяца: `зарплата/аванс + отпускные + прочий доход
(таблица income) − расходы − плановый платёж по долгу`. Платёж по долгу
(`debts.monthly_payment`, по `debts.payment_half`) вычитается только за
месяцы, где долг «жив»: от месяца создания до ориентировочного месяца
погашения по плановой ставке (`_debt_payment_active`). Несколько
источников дохода на одну половину суммируются; повторяющиеся доходы
проецируются вперёд как и расходы. Расчёт зарплаты с отпуском — по ТК РФ
для ВСЕХ методов распределения: оклад урезается пропорционально
отработанным дням месяца, отпускные начисляются отдельной суммой;
граница «первая/вторая половина» — по `advance_cutoff_day`
(`SalaryCalculator._day_in_first_half`), не по захардкоженному 15-му.
НДФЛ: плоская ставка `taxRate` либо (переключатель `taxProgressive`)
прогрессивная шкала 2025 — предельный налог месяца = НДФЛ с дохода
нарастающим итогом по этот месяц минус по предыдущий
